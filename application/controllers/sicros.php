            ...
            */
            ...
            if($search_string){
                if($order){
                    ...
                    $data['sicros'] = $this->sicrosdao->get_sicros($search_string, 'data_base', $order_type, $config['per_page'],$limit_end);           
                }
            }else{
                if($order){
                    $data['sicros'] = $this->sicrosdao->get_sicros('', $order, $order_type, $config['per_page'],$limit_end);        
                }else{
                    $data['sicros'] = $this->sicrosdao->get_sicros('', 'data_base', $order_type, $config['per_page'],$limit_end);        
                }
            }
...
public function add()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
        	...
        	$this->form_validation->set_rules('indice_pavimentacao_base', 'indice_pavimentacao_base', 'numerico|required');
        	$this->form_validation->set_rules('indice_pavimentacao', 'indice_pavimentacao', 'numerico|required'); 
        	$this->form_validation->set_rules('bdi', 'bdi', 'numerico|required'); 
        	$this->form_validation->set_rules('bdi_betuminosos', 'bdi_betuminosos', 'numerico|required');
        	$this->form_validation->set_rules('icms_asfaltico', 'icms_asfaltico', 'numerico|required');
            ...
            if ($this->form_validation->run())
            {
                $data_to_store = array('titulo' => $this->input->post('titulo'),
                		...
                		'indice_pavimentacao_base' => $this->input->post('indice_pavimentacao_base'),
                		...
                		'icms_asfaltico' => $this->input->post('icms_asfaltico'),
