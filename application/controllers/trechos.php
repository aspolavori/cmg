<?php
    			...
    			class trechos extends App_controller {
    			const VIEW_FOLDER = 'admin/trechos';
    		public function __construct()
		    {
		        parent::__construct();
		        $this->load->model('trechosdao');
    ...
    public function index()
    {
        ...
        $config['base_url'] = base_url().'admin/trechos';
        ...
        if($search_string !== false && $order !== false || $this->uri->segment(3) == true){ 
            ...
            */
            ...
            $data['count_products']= $this->trechosdao->count_trechos($search_string, $order);
            $config['total_rows'] = $data['count_products'];

            //fetch sql data into arrays
            if($search_string){
                if($order){
                    $data['trechos'] = $this->trechosdao->get_trechos($search_string, $order, $order_type, $config['per_page'],$limit_end);        
                }else{
                    $data['trechos'] = $this->trechosdao->get_trechos($search_string, '', $order_type, $config['per_page'],$limit_end);           
                }
            }else{
                if($order){
                    $data['trechos'] = $this->trechosdao->get_trechos('', $order, $order_type, $config['per_page'],$limit_end);        
                }else{
                    $data['trechos'] = $this->trechosdao->get_trechos('', '', $order_type, $config['per_page'],$limit_end);        
                }
            }
            ...
            $filter_session_data['trecho_selected'] = null;
            ...
            //fetch sql data into arrays
            $data['count_products']= $this->trechosdao->count_trechos();
            $data['trechos'] = $this->trechosdao->get_trechos('', '', $order_type, $config['per_page'],$limit_end);        
            $config['total_rows'] = $data['count_products'];
        ...
        //load the view
        $data['main_content'] = 'admin/trechos/list';
...
public function add()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            if ($this->form_validation->run())
            {
                ...
                if($this->trechosdao->store_trecho($data_to_store)){
        ...
        //load the view
        $data['main_content'] = 'admin/trechos/add';
        $this->load->view('includes/template', $data);  
    }       
    ...
    public function update()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
        	...
        	'ID_TP_TRECHO' => $this->input->post('ID_TP_TRECHO'),                    
                ...
                //if the insert has returned true then we show the flash message
                if($this->trechosdao->update_trecho($id, $data_to_store) == TRUE){
                    $this->session->set_flashdata('flash_message', 'updated');
                }else{
                    $this->session->set_flashdata('flash_message', 'not_updated');
                }
                redirect('admin/trechos/update/'.$id.'');
        ...
        //product data 
        $data['trecho'] = $this->trechosdao->get_trecho_by_id($id);
        //load the view
        $data['main_content'] = 'admin/trechos/edit';
        $this->load->view('includes/template', $data);            
    ...
    */
    public function delete()
    {
        //product id 
        $id = $this->uri->segment(4);
        $this->trechosdao->delete_trecho($id);
        redirect('admin/trechos');
    }//edit    			
    	}