<?php
    			...
    			class gps_logs extends App_controller {
    			const VIEW_FOLDER = 'admin/gps_logs';
    		public function __construct()
		    {
		        parent::__construct();
		        $this->load->model('gps_logsdao');
    ...
    public function index()
    {
        ...
        $config['base_url'] = base_url().'admin/gps_logs';
        ...
        if($search_string !== false && $order !== false || $this->uri->segment(3) == true){ 
            ...
            */
            ...
            $data['count_products']= $this->gps_logsdao->count_gps_logs($search_string, $order);
            $config['total_rows'] = $data['count_products'];

            //fetch sql data into arrays
            if($search_string){
                if($order){
                    $data['gps_logs'] = $this->gps_logsdao->get_gps_logs($search_string, $order, $order_type, $config['per_page'],$limit_end);        
                }else{
                    $data['gps_logs'] = $this->gps_logsdao->get_gps_logs($search_string, '', $order_type, $config['per_page'],$limit_end);           
                }
            }else{
                if($order){
                    $data['gps_logs'] = $this->gps_logsdao->get_gps_logs('', $order, $order_type, $config['per_page'],$limit_end);        
                }else{
                    $data['gps_logs'] = $this->gps_logsdao->get_gps_logs('', '', $order_type, $config['per_page'],$limit_end);        
                }
            }
            ...
            $filter_session_data['gps_log_selected'] = null;
            ...
            //fetch sql data into arrays
            $data['count_products']= $this->gps_logsdao->count_gps_logs();
            $data['gps_logs'] = $this->gps_logsdao->get_gps_logs('', '', $order_type, $config['per_page'],$limit_end);        
            $config['total_rows'] = $data['count_products'];
        ...
        //load the view
        $data['main_content'] = 'admin/gps_logs/list';
...
public function add()
    {
        ...
        //load the view
        $data['main_content'] = 'admin/gps_logs/add';
        $this->load->view('includes/template', $data);  
    }       
    ...
    public function update()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
        	...
        	'ID_LOG_ORIGINAL' => $this->input->post('ID_LOG_ORIGINAL'),                    
                ...
                //if the insert has returned true then we show the flash message
                if($this->gps_logsdao->update_gps_log($id, $data_to_store) == TRUE){
                    $this->session->set_flashdata('flash_message', 'updated');
                }else{
                    $this->session->set_flashdata('flash_message', 'not_updated');
                }
                redirect('admin/gps_logs/update/'.$id.'');
        ...
        //product data 
        $data['gps_log'] = $this->gps_logsdao->get_gps_log_by_id($id);
        //load the view
        $data['main_content'] = 'admin/gps_logs/edit';
        $this->load->view('includes/template', $data);            
    ...
    */
    public function delete()
    {
        //product id 
        $id = $this->uri->segment(4);
        $this->gps_logsdao->delete_gps_log($id);
        redirect('admin/gps_logs');
    }//edit    			
    	}