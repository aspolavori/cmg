...
class sicro_mao_obra_custodao extends App_DAO {
	...
	const VIEW_FOLDER = 'admin/sicro_mao_obra_custo';
	    ...
	    */
	    ...
	    public function get_sicro_mao_obra_custo_by_id_sicro($id)
	    {
	    	 
	    	$this->db->select($this->table.'.*, '.$this->maoobraTable.'.codigo, '.$this->maoobraTable.'.titulo ' );
	    	$this->db->from($this->sicroTable);
	    	$this->db->join($this->table, $this->sicroTable.'.id = '.$this->table.'.id_sicro' , 'inner');
	    	$this->db->join($this->maoobraTable, $this->maoobraTable.'.id = '.$this->table.'.id_mao_obra' , 'inner');
	    	$this->db->where($this->sicroTable.'.id', $id );
	    	$this->db->order_by($this->maoobraTable.'.codigo' , 'ASC');
	    ...
	    public function get_sicro_mao_obra_custo_not_defined_by_id_sicro($id)
	    {
	    	 
	    	$query = 'select '.$this->maoobraTable.'.* from '.$this->maoobraTable.' where '.$this->maoobraTable.'.id NOT IN ('
	    			.'select '.$this->maoobraTable.'.id from '.$this->sicroTable.'
							inner join '.$this->table.' on ('.$this->sicroTable.'.id = '.$this->table.'.id_sicro)
							inner join '.$this->maoobraTable.' on ('.$this->table.'.id_mao_obra = '.$this->maoobraTable.'.id )
