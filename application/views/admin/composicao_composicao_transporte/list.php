	  ...
	  <div class="row">
        <div class="span12 columns">
          <div class="well">
            ...
            $options_composicao_composicao_transporte = array();    
            foreach ($composicao_composicao_transporte as $array) {
              foreach ($array as $key => $value) {
                $options_composicao_composicao_transporte[$key] = $key;
              }
              break;
            }

            echo form_open("admin/composicao_composicao_transporte", $attributes);
     
              echo form_label("Search:", "search_string");
              echo form_input("search_string", $search_string_selected);

              echo form_label("Order by:", "order");
              echo form_dropdown("order", $options_composicao_composicao_transporte, $order, 'class="span2"');
          ...
          <table class="table table-striped table-bordered table-condensed">
            <thead>
              <tr>
				...
				<th class="yellow header headerSortDown">Quantideda</th>
	            ...
	            <tbody>
	              ...
	              foreach($composicao_composicao_transporte as $row)
	              {
					...
					echo '<td>'.$row['id_composicao_transporte'].'</td>';
					echo '<td>'.$row['quantidade'].'</td>';
					echo '<td>'.$row['observacao'].'</td>';
					
	          echo '<td class="crud-actions">
                  <a href="'.site_url("admin").'/composicao_composicao_transporte/update/'.$row['id'].'" class="btn btn-info">Ver & editar</a>  
                  <a href="'.site_url("admin").'/composicao_composicao_transporte/delete/'.$row['id'].'" class="btn btn-danger">deletar</a>
                </td>';
                echo "</tr>";
              }
              ?>      
