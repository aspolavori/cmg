	  ...
	  <div class="row">
        <div class="span12 columns">
          <div class="well">
            ...
            echo form_open("admin/consulta_contagens", $attributes);
         ...
         <table class="table table-striped table-bordered table-condensed">
            <thead>
              <tr>
				...
				<th class="yellow header headerSortDown">Número do Posto</th>
				<th class="yellow header headerSortDown">Chefe do Posto</th>
	            ...
	            <tbody>
	              ...
	              foreach($pesquisas as $row)
	              {
					...
					echo '<td>'.$row['km'].'</td>';
					echo '<td>'.$row['n_posto'].'</td>';
					echo '<td>'.$row['chefe_posto'].'</td>';
					echo '<td>'.$row['data_ini'].'</td>';
					echo '<td>'.$row['data_fim'].'</td>';
					
	        	   echo '<td class="crud-actions">';
	        	   if(array_key_exists('contagem', $row)){
	        	  	echo '<a href="'.site_url("admin").'/consulta_contagens/resultado/'.$row['id'].'" class="btn btn-info" style="width:100px;" >Exibir Contagem</a> </br>';  
	        	   }
	        	   if(array_key_exists('od', $row)){
	        	  	echo '<a href="'.site_url("admin").'/consulta_contagens/resultado_od/'.$row['id'].'" class="btn btn-info" style="width:100px;" >Exibir ODs</a> </br>'; 	
	        	   }
                	echo '</td>';
                echo "</tr>";
              }
...
$(function(){
		 ...
		 $('select[name=uf]').change(function(){
		  ...
		  $.getJSON( "<?php echo base_url().'consulta_contagens/get_brs_by_uf/' ?>" + id , function( data ) {
		 ...
		 $('select[name=br]').change(function(){
			  ...
			  $.getJSON( "<?php echo base_url().'consulta_contagens/get_kms_by_uf_br/' ?>" + ufNome + "/" + id , function( data ) {
