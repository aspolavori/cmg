		  ...
		  <ul class="breadcrumb">
	       ...
	       <?php 
	       		foreach($classeVeiculos as $item){	
	          		echo '<a  href=" '.site_url("admin").'/'.$this->uri->segment(2).'/resultado_od/'.$pesquisa_trafego['id'].'/'.$item['id'].'" class="btn btn-success" style="margin-right: 2px;">'.$item['titulo'].'</a>';
         ...
         <table class="table table-striped table-bordered table-condensed">
            <thead>
              <tr>
				...
				<?php
					foreach( $perguntas_adicionais as $row){
						echo '<th class="yellow header headerSortDown">'.$row['titulo'].'</th>';
					} 
				?>
			</tr>
	            </thead>
	            <tbody>
	              ...
	              foreach($origens_destinos as $row)
	              {
					...
					foreach($row['perguntas'] as $row2){
						if($row2['resposta']){
							echo '<td>'.$row2['resposta'].'</td>';
						}else{
							echo '<td></td>';
						}
