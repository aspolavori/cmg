          ...
          <table class="table table-striped table-bordered table-condensed">
            <thead>
              <tr>
	            ...
	            <tbody>
	              <?php
	              foreach($sicro_mao_obra_custo_not_defined as $row)
	              {
	              	echo '<tr>';
	              	echo '<td>'.$row['id'].'</td>';
	              	echo '<td>'.$row['codigo'].'-'.$row['titulo'].'</td>';
	              	echo '<td colspan="2" style="color:red;">Essa Mão de Obra consta no Banco de Mão de Obra mas não possui valor associado.</td>';
	              	 
	              	echo '<td class="crud-actions">
	                  <a href="'.site_url("admin").'/sicro_mao_obra_custo/add/'.$id_sicro.'/'.$row['id'].'" class="btn btn-success">Adicionar</a>
	                </td>';
	              	echo "</tr>";
	              }
	          ...
	          echo '<td class="crud-actions">
                  ...
                  <a href="'.site_url("admin").'/sicro_mao_obra_custo/delete/'.$row['id'].'/'.$id_sicro.'" class="btn btn-danger">deletar</a>
                </td>';
                echo "</tr>";
              }
              ?>      
